...
extension SelectedColorViewController {
    func bind() {
        ...
        let datasource: RxCollectionViewSectionedAnimatedDataSource<Section> =
            RxCollectionViewSectionedAnimatedDataSource(configureCell: { (datasource, collectionView, indexPath, color) -> UICollectionViewCell in
        ...
        reverseButton.rx.tap.subscribe(onNext: {
            [weak self] _ in
        ...
        collectionView.refreshControl?.rx.controlEvent(UIControlEvents.valueChanged)
            .subscribe(onNext: {
                [weak self] _ in
                guard let `self` = self else {return}
                let colors = (try? self.colors.value()) ?? []
